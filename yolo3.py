...
aipp_flag = True
model_path = './Yolov3_person_head.om'
...
def draw_box_on_img(img_data, res):
    ...
    for bbox in res:
        ...
        if score > conf_threshold:     # 闃堝€?            cv2.rectangle(img_data, (x_min, y_min), (x_max, y_max), colors[label], thickness)
            s = '%.1f%%' % (score*100)
...
def handler(a, b):
    ...
    url = "rtsp://admin:Hilens123@192.168.137.40:554/h264/ch1/main/av_stream" 
    camera = hilens.VideoCapture(url)
...
if __name__ == '__main__':
    ...
    #image_test()   # 鍗曞紶鍥剧墖娴嬭瘯

    handler(None, None)   # 瀹炴椂妫€娴?
